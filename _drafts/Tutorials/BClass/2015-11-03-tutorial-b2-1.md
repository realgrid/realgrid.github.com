...
행 고정 옵션(FixedOptions)은 아래와 같습니다.
...
행(Row)고정 외에 열(Column)고정 옵션에 대해서는 [FixedOptions](/api/types/FixedOptions/)에서 확인 하실 수 있습니다.

아래 실습을 통하여 [FixedOptions](/api/types/FixedOptions/)의 설정에 따른 상태변화 모습을 확인하세요.
...
4. 버튼을 클릭하면 고정된 행의 데이터를 필터링에서 제외 시키기 위해 아래 코드를 추가합니다.
	<pre class="prettyprint">
	$("#btnExceptFormSorting").on("click", function(){
        gridView.setFixedOptions({
