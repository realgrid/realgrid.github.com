...
로우 그루핑(row grouping)이란 특정 컬럼을 기준으로 그룹핑하는 것을 의미합니다. 즉, 해당 컬럼의 데이터값이 동일한 행을 묶는 것을 의미합니다. [[A15 강좌](/tutorial/a15/)],[[A16 강좌](/tutorial/a16/)]에서 로우 그루핑(row grouping)에 대한 기본적인 이론과 사용방법을 배웠습니다. 이 강좌에서는 rowGroup.header, rowGroup.footer, panel의 3가지 영역에 데이터를 표시하는 방법에 대한 옵션에 대하여 배워보도록 하겠습니다.

**이 강좌를 연습하기 전에 RealGrid의 [Style Regions](http://demo.realgrid.com/Demo/StylesRegions)에서 rowGroup영역들을 이해하고 넘어가면 좋을 것 같습니다. [링크](http://demo.realgrid.com/Demo/StylesRegions)로 이동하여 rowGroup의 모든영역의 명칭을 확인하세요.**
...
[RowGroupOptions](http://help.realgrid.com/api/types/RowGroupOptions/)의 옵션은 아래와 같습니다.
...
* summaryMode : 각 행 그룹의 합계를 구하는 방식입니다. rowGroup.footer영역에 합계, 분산 등 내용 표시방법을 설정할 때 사용합니다.
* levelIndent : rowGroup.bar영역의 너비를 설정할 수 있습니다.
...
* headerStatement: "`문자`"
  * 모든 rowGroup.header영역에 `문자` 이라는 문자열 값을 표시한다.
...
* headerStatement: “${groupField}: ${groupValue} - ${rowCount} rows”
  * 다음과 같이 여러 속성을 한번에 사용할 수 도있습니다.
  * 문자열 " "안에 영역에 문자를 넣어서 표시할 수 있습니다.

그리고 [gridView.setGroupingOptions()](http://help.realgrid.com/api/GridBase/setGridOptions/)에서는 panel에 메시지를 표시할 수 있으며 그루핑을 할 수 없도록 할 수도 있습니다.
...
아래 실습을 통하여 rowGroup.header영역에 표시되는 값들이 변하는 모습을 확인하세요.
...
12. <button type="button" class="btn btn-primary btn-xs" id="btnexpandWhenGrouping">필드 접기</button>버튼을 클릭고 그루핑하면 모든 필드가 접힌상태로 그루핑됩니다. 다른 컬럼을 그루핑하여 결과를 확인합니다.

13. <button type="button" class="btn btn-primary btn-xs" id="btnGroupenabled">그루핑 불가능</button>버튼을 클릭하면 그루핑을 할 수 없도록 설정합니다. 그루핑을 시도하여 결과를 확인합니다.
...
13. &lt;button type=&quot;button&quot; class=&quot;btn btn-primary btn-xs&quot; id=&quot;btnGroupenabled&quot;&gt;그루핑 불가능&lt;/button&gt;버튼을 클릭하면 그루핑 불가 상태로 설정합니다. 그루핑을 시도하여 결과를 확인합니다.
...
* [GroupingOption](http://help.realgrid.com/api/types/GroupingOptions/)
* [Row Grouping](http://help.realgrid.com/api/features/Row%20Grouping/)
* [getGroupingOptions](http://help.realgrid.com/api/GridBase/getGroupingOptions/)
* [setGroupingOptions](http://help.realgrid.com/api/GridBase/setGroupingOptions/)
* [getGroupSummary](http://help.realgrid.com/api/GridBase/getGroupSummary/)