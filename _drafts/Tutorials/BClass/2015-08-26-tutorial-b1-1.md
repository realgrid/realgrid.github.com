...
1. 먼저 알아두셔야 할 점
    ...
    다른 대부분의 함수의 경우도 거의 동일하지만 이 강좌에 나오는 함수에 전달하는 column 매개변수에는 column.name 또는 column object를 전달할 수 있습니다. 예를들어 toggleColumnFilters 함수의 경우 
    ...
    <pre class="prettyprint">
    var col = gridView.columnByName("textColumn");
...
2. [Filter](/api/types/ColumnFilter/){:target="_blank"}를 생성하는 방법은 다음과 같습니다.
  ...
  * [GridBase.addColumnFilters()](/api/GridBase/addColumnFilters/){:target="_blank"}함수로 Filter 추가
    기존 Filter는 유지되고 동일한 이름일 경우 overwrite parameter에 따라 기존 Filter를 지우고 다시 생성됩니다.

    <pre class="prettyprint">
    gridView.addColumnsFilters(column, filters, true);</pre>
...
<label for="btnActivateFilter">B, C Filter 활성화</label></li>
...
<label for="btnDeactivateFilter">B, C Filter 비활성화</label></li>
