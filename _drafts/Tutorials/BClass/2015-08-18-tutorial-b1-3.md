...
FilterAction에 "사용자 정의 필터" 항목을 추가하여 이 FilterAction을 클릭하였을 때 별도의 Dialog에서 수식을 사용자가 선택하여 Filter로 추가하는 방법으로 구현하겠습니다.
...
3. 각 컬럼에 "사용자 정의 필터" FilterAction 추가
    ...
    [GridBase.setColumnFilterActions()](/api/GridBase/setColumnFilterActions/){:target="_blank"}함수를 사용하여 각 컬럼에 FilterAction을 추가합니다.
...
6. Filter Expression 조합

    여기에서 dialog_condition1와 dialog_condition2는 비교 부호 dropdown, dialog_value1와 dialog_value2는 입력값 Textbox, dialog_logicOp는 논리연산자(그리고, 또는)을 선택하는 control입니다.
    이 강좌에서는 환경적으로 dropdown의 값 <, >가 자동으로 Encode 되어서 이를 Decode하였으나, 실제 웹서버 환경에서는 다를 수 있습니다. 
    dropdown의 내용은 다음과 같습니다.
...
7. Filter 등록
    ...
    Filter를 등록하기에 앞서 다른 필터가 존재할 경우 결과가 다르게 나올 수 있으므로 [GridBase.removeColumnFilters()](/api/GridBase/removeColumnFilters/){:target="_blank"}을 사용하여 모든 컬럼의 기존 custom_filter를 제거하고, [GridBase.activateAllColumnFilters()](/api/GridBase/activateAllColumnFilters/){:target="_blank"}을 사용하여 현재 활성화된 다른 filter들을 비활성화시켜야 합니다. 
